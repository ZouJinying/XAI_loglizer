...
class FeatureExtractor(object):
    ...
    def fit_transform(self, X_seq, term_weighting=None, normalization=None, oov=False, min_count=1):
        ...
        Arguments
        ---------
            X_seq: ndarray, log sequences matrix  对数序列矩阵
            term_weighting: None or `tf-idf` ##tf_idf中tf为词频，idf为逆文档频率（log（（语料库的文档总数）/（包含该词汇的文档数+1））
        ...
        self.oov = oov## ovv_未登录词，未登录词就是训练时未出现，测试时出现了的单词。

        X_counts = []
        for i in range(X_seq.shape[0]):##shape[0]为矩阵列得维数
